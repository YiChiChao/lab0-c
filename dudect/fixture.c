...
typedef int (*cmp_func_t)(int (*)(const void *, const void *));
...
static t_context_t *max_test()
{
    ...
    for (size_t i = 0; i < DUDECT_TESTS; i++) {
        if (t[i]->n[0] > DUDECT_ENOUGH_MEASUREMENTS) {
...
{
    t_context_t *mattest_t = max_test();
    double max_t = fabs(t_compute(mattest_t));
    double number_traces_max_t = mattest_t->n[0] + mattest_t->n[1];
...
static bool doit(int mode)
{
    ...
    // test the fix test (see if insert or remove one element is correctly
    // executing)
